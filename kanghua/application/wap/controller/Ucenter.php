...
//dump($res);die;
    ...
    public function upgradepay(){
        ...
        try{
            ...
            if (!$res){
                Logs::writeMongodb(200010,'db_orders',$ordersid,'创建订单信息失败，请重试',$orders,'Ym','fenxiao_orders',Session::get('user.username'));
            ...
            if (!$res1){
                Logs::writeMongodb(200010,'db_orders_goods',$ordersid,'创建订单商品记录失败，请重试',$orders_goods,'Ym','fenxiao_orders',Session::get('user.username'));
    ...
    public function withdraw(){
        ...
        if(Request::instance()->isPost()){
            ...
            if(getSuperMD5($password . $account['a_payPwdNew']) != $account['a_payPwd']){
                ...
                Logs::writeMongodb(400000,'db_account_cash_out',Session::get('user.username'),'提现密码错误！',$record,'','fenxiao_account');
                $lastCount = 3 - ($count + 1);
            ...
            try{
                ...
                if(!$res){
                    Logs::writeMongodb(800000,'db_account_cash_out',Session::get('user.username'),'创建资金出账表失败',$data,'Ym','fenxiao_account');
                ...
                if(!$res){
                    Logs::writeMongodb(800000,'db_account_cash_tran',Session::get('user.username'),'创建资金进出账异动表失败',$data,'Ym','fenxiao_account');
                ...
                if(!$res){
                    Logs::writeMongodb(800000,'db_account',Session::get('user.username'),'更新账户失败',$account,'Ym');
                    throw new Exception('更新账户失败');
                }

                Db::commit();
                Logs::writeMongodb(800001,'db_account',Session::get('user.username'),'账户提现成功',$account,'Ym');
