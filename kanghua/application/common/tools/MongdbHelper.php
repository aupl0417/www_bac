 ...
 **/
//参数：
//"array('username' => new mongoRegex("/yhp.*/"))"
...
class MongdbHelper
{
    ...
    {
        $this->_config =Config::get('mongo');
        $server = sprintf("mongodb://%s:%s@%s:%s/%s", $this->_config['username'], $this->_config['password'], $this->_config['hostname'], $this->_config['hostport'], $this->_config['database']);
...
//        echo "connect\n";
    ...
    public function name($name){
        $this->table($this->_config['prefix'].$name);
        return $this;
    }
     ...
     * @return boolean
     */
    public function insert($sets, $safe = false, $fsync = false)
    {
        $col = $this->_getCol($this->_table);
        try {
     ...
     * @return boolean
     */
    public function save($sets, $safe = false, $fsync = false)
    {
        // 处理 '_id' 字段
        $sets = $this->_parseId($sets);
        $ret = $this->_getCol($this->_table)->save($sets, array('w' => $safe, 'fsync' => $fsync));
        return $ret;
    }
     ...
     * @return boolean
     */
    public function delete($query = array(), $delAll = true, $safe = false, $fsync = false)
    {
        // 自动处理 '_id' 字段
    ...
    {
        return $this->_getCol($this->_table)->drop();
    }
     ...
     * @return boolean
     */
    public function update($newDoc, $query = array(), $option = 'set', $upAll = true, $upsert = false, $safe = false, $fsync = false)
    {
        // 自动处理 '_id' 字段
        $query = $this->_parseId($query);
        // 得到集合
    ...
    {
        return $this->_getCol($this->_table)->count($query, $limit, $skip);
    }
     ...
     * @return array
     */
    public function fetchRow($query = array(), $fields = array())
    {
        // 得到集合名
        $col = $this->_getCol($this->_table);
        // 自动处理 '_id' 字段
     ...
     * @return mixed
     */
    public function fetchOne($query = array(), $fields = '_id')
    {
        $ret = $this->fetchRow($this->_table, $query, array($fields));
     ...
     * @return array
     */
    public function fetchCol($query = array(), $fields = '_id')
    {
        $result = array();
        $list = $this->select($this->_table, $query, array($fields));
        foreach ($list as $row) {
            $result[] = $row[$fields];
        }
        return $result;
     ...
     * @return array
     */
    public function fetchAssoc($query = array(), $fields = '_id')
    {
        $result = array();
        $list = $this->select($this->_table, $query);
        foreach ($list as $row) {
            $key = $row[$fields];
            $result[][$key] = $row;
        }
