...
+function ($) {
	...
	Dialog.prototype.doAjaxChecked = function(options) {
    ...
    $(document).on('click.bjui.dialog.data-api', '[data-toggle="dialogcheck"]', function(e) {
        var $this   = $(this), href = $this.attr('href'), data = $this.data(), options = data.options,
        $target = null, $element = $this
        if (options) {
            if (typeof options == 'string') options = options.toObj()
            if (typeof options == 'object')
                $.extend(data, options)
		...
		if (!data.url) {
            ...
            // data.url = decodeURI(data.url).replacePlh($element.closest('.unitBox'))
            
            /* if (!data.url.isFinishedTm()) {
                $element.alertmsg('error', (data.warn || FRAG.alertPlhMsg.replace('#plhmsg#', BJUI.regional.plhmsg)))
                BJUI.debug('The ajax url is incorrect: '+ data.url)
                return
            } */
        }
        
		if (!data.group) {
                $element.alertmsg('error', data.warn || FRAG.alertNoCheckGroup.replace('#nocheckgroup#', BJUI.regional.nocheckgroup))
                e.preventDefault()
                return
        }
       /*  if (!$target || !$target.length) {
        ...
        $target = $.CurrentNavtab
        var ids      = []
        var $checks  = $target.find(':checkbox[name='+ data.group +']:checked')
        //var callback = data.callback && data.callback.toFunc()
        if ($checks.length == 0) {
            $element.alertmsg('error', FRAG.alertNotChecked.replace('#notchecked#', BJUI.regional.notchecked))
            e.preventDefault()
            return
        ...
        data.url = data.url.setUrlParam((data.idname ? data.idname : 'ids'), ids.join(','))
        Plugin.call($this, data)
