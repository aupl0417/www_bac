...
$(function() {
    BJUI.init({
        ...
        loginInfo    : { url:'admin/publics/login', title:'登录', width:400, height:200}, // 会话超时后弹出登录对话框
        ...
        <div id="bjui-hnav">
            ...
            <div id="bjui-hnav-navbar-box">
                <ul id="bjui-hnav-navbar">
                    {notempty name="menu"}
                    {foreach name="menu" key='k' item="one"}
                    <li {eq name="k" value="0"}class="active"{/eq}><a href="javascript:;" data-toggle="slidebar"><i class="fa fa-{$one.icon|default='cog'}"></i> {$one.name}</a>
                                {notempty name="one._child"}
                                    <div class="items hide" data-noinit="true">
                                        <ul class="menu-items" data-faicon="table">
                                            {foreach name="one._child" item="two"}
                                            {notempty name="two._child"}
                                                <li class="active switch open"><a href="javascript:;"><i class="fa fa-{$two.icon|default='list-ol'}"></i> {$two.name}</a>
<!--                                                    <a href="javascript:;"  data-options="{faicon:'{$two.icon|default='cog'}'}">{$two.name}</a>-->
                                                    <b><i class="fa fa-angle-down"></i></b>
                                                    <ul class="menu-items-children" style="display: block">
                                                        {foreach name="two._child" item="three"}
                                                        <li><a href="{:url($three.module.'/'.$three.controller.'/'.$three.action)}" data-options="{id:'{$three.module}_{$three.controller}_{$three.action}', faicon:'{$three.icon|default='caret-right'}'}">{$three.name}</a></li>
                                                        {/foreach}
                                                    </ul>
                                                </li>
                                            {else /}
                                            <li><a href="{:url($two.module.'/'.$two.controller.'/'.$two.action)}" data-options="{id:'{$two.module}_{$two.controller}_{$two.action}', faicon:'{$two.icon|default='caret-right'}'}">{$two.name}</a>
                                            {/notempty}
                                            {/foreach}
                                        </ul>
                                    </div>
