...
<!-- for doc begin -->
<link type="text/css" rel="stylesheet" href="/static/js/syntaxhighlighter-2.1.382/styles/shCore.css"/>
<link type="text/css" rel="stylesheet" href="/static/js/syntaxhighlighter-2.1.382/styles/shThemeEclipse.css"/>
<script type="text/javascript" src="/static/js/syntaxhighlighter-2.1.382/scripts/brush.js"></script>
...
<!--            <a class="bjui-navbar-logo" href="#"><img src="/static/images/logo.png"></a>-->
        ...
        <nav id="bjui-navbar-collapse">
            <ul class="bjui-navbar-right">
                ...
                <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown">{$Think.cookie.username} <span class="caret"></span></a>
                    <ul class="dropdown-menu" role="menu">
                        <li><a href="{:url('System/changePassword')}" data-toggle="dialog" data-id="System_changePassword" data-mask="true" data-width="600" data-height="260">&nbsp;<span class="glyphicon glyphicon-lock"></span> 修改密码&nbsp;</a></li>
                        <li><a href="{:url('System/profile')}" data-toggle="dialog" data-id="System_profile" data-mask="true" data-width="600" data-height="260">&nbsp;<span class="glyphicon glyphicon-user"></span> 我的资料</a></li>
                        <li class="divider"></li>
                        <li><a href="{:url('Publics/logout')}" class="red">&nbsp;<span class="glyphicon glyphicon-off"></span> 注销登陆</a></li>
                    </ul>
                </li>
        ...
        <div id="bjui-hnav">
            ...
            <div id="bjui-hnav-navbar-box">
                <ul id="bjui-hnav-navbar">
                    {notempty name="menu"}
                        {foreach name="menu" item="oneLevel" key="oneKey"}
                            <li {eq name="oneKey" value="0"} class="active"{/eq}><a href="javascript:;" data-toggle="slidebar"><i class="fa fa-{$oneLevel.icon|default='cog'}"></i> {$oneLevel.name}</a>
                                <div class="items hide" data-noinit="true">
                                    {notempty name="oneLevel._child"}
                                    {foreach name="oneLevel._child" item="twoLevel"}
                                        {notempty name="twoLevel._child"}
                                        <ul class="menu-items" data-title="{$twoLevel.name}" data-faicon="{$twoLevel.icon|default='cog'}">
                                            {foreach name="twoLevel._child" item="threeLevel"}
                                            {notempty name="threeLevel._child"}
                                            <li><a href="{:url($threeLevel['module'].'/'.$threeLevel['controller'].'/'.$threeLevel['action'])}" data-options="{id:'{$threeLevel['module']}_{$threeLevel['controller']}_{$threeLevel['action']}', faicon:'{$threeLevel.icon|default='cog'}'}" data-fresh="true">{$threeLevel.name}</a><b><i class="fa fa-angle-down"></i></b>
                                                <ul class="menu-items-children">
                                                </ul>
                                            </li>
                                            {else /}
                                            <li><a href="{:url($threeLevel['module'].'/'.$threeLevel['controller'].'/'.$threeLevel['action'])}" data-options="{id:'{$threeLevel['module']}_{$threeLevel['controller']}_{$threeLevel['action']}', faicon:'{$threeLevel.icon|default='cog'}'}" data-fresh="true">{$threeLevel.name}</a></li>
                                            {/notempty}
                                            {/foreach}
                                        </ul>
                                        {else /}
                                        <ul class="menu-items" data-faicon="cog"}">
                                            <li><a href="{:url($twoLevel['module'].'/'.$twoLevel['controller'].'/'.$twoLevel['action'])}" data-options="{id:'{$twoLevel['module']}_{$twoLevel['controller']}_{$twoLevel['action']}', faicon:'{$twoLevel.icon|default='cog'}'}" data-fresh="true">{$twoLevel.name}</a></li>
                                        </ul>
                                        {/notempty}
                                        {/foreach}
                                    {/notempty}
            ...
            <div class="tabsPageHeader">
                <div class="tabsPageHeaderContent">
                    <ul class="navtab-tab nav nav-tabs">
                        <li data-url="{:url('welcome')}" data-faicon="home"><a href="javascript:;"><span><i class="fa fa-home"></i> #maintab#</span></a></li>
                    </ul>
                </div>
