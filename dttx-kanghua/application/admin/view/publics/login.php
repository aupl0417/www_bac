...
.login_logo {margin-bottom: 30px;text-align: center;}
.login_logo img {}
...
$(function() {
    ...
    $("#login_form").submit(function(){
        ...
        $("#login_ok").attr("disabled", true).val('{:lang('connecting_please_wait')}');
...
function changeCode(){
    $("#captcha_img").attr("src", "{:url('Publics/verfy')}");
}
function choose_bg() {
...
function check_username(){
    ...
    if(username != ''){
        $.ajax({
            url: '/admin/Publics/ajax_check_username/',
            type: 'POST',
            dataType: 'JSON',
            data:{
                username:username
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.flag){
                ...
                location.href = "{:url('/admin/admincp')}";
            }else{
                $(".login_error").text(data.message).hide().fadeIn();
                $("#login_ok").attr("disabled", false).val('{:lang('login')}');
                check_username();
            }
        },
        error: function(){
        ...
        <div class="login_form">
            ...
            <form action="{:url('')}" id="login_form" method="post">
                ...
                <div class="form-group">
                    <label for="j_username" class="t">{:lang('username')}：</label> <input id="j_username" value="" name="username" type="text" class="form-control x319 in" autocomplete="off">
                </div>
                <div class="form-group">
                    <label for="j_password" class="t">{:lang('password')}：</label> <input id="j_password" value="" name="password" type="password" class="form-control x319 in">
                </div>
                <div class="form-group" id="verify" style="display:none;">
                    <label for="j_captcha" class="t">{:lang('checkcode')}：</label> <input id="j_captcha" name="code" type="text" class="form-control x164">
                    <img id="captcha_img" alt="{:lang('click_change_validate')}" title="{:lang('click_change_validate')}" src="#" class="m">
                </div>
                <div class="form-group">
                    <label class="t"></label>
                    <label for="j_remember" class="m"><input id="j_remember" type="checkbox" value="true">&nbsp;{:lang('remember')}!</label>
                </div>
                <div class="form-group space">
                    <label class="t"></label>　　　
                    <input type="submit" id="login_ok" value="&nbsp;{:lang('login')}&nbsp;" class="btn btn-primary btn-lg">&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="reset" value="&nbsp;{:lang('reset')}&nbsp;" class="btn btn-default btn-lg">
                </div>
            </form>
        </div>
    </div>
    <div class="bottom">Copyright &copy; 2016 - 2018 </div>
</div>
</body>
</html>