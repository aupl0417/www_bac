<div class="bjui-pageHeader">
    <form id="pagerForm" data-toggle="ajaxsearch" action="{:url('')}" method="post">
        ...
        <div class="bjui-searchBar">
            <label>用户名：</label><input type="text" id="username" value="{$username|default=""}" name="username" class="form-control" size="10">&nbsp;
            <label>角色:</label>
            <select name="roleid" data-toggle="selectpicker">
                <option value="">所有角色</option>
                {foreach name="roles" key="key" item="role"}
                <option value="{$key}" {notempty name="roleid"}{eq name="key" value="$roleid"}selected{/eq} {/notempty}>{$role.rolename}</option> 
                {/foreach}
            </select>&nbsp;
    ...
    <table data-toggle="tablefixed" data-width="100%" data-nowrap="true">
        ...
        <tbody>
            ...
            <tr data-id="{$item.userid}">
                ...
                <td>{$roles[$item['roleid']]['rolename']|default=""}</td>
                <td>{$item.nickname|default=""}</td>
                <td align="center">{$item.lastloginip|default=""}</td>
                <td align="center">{$item.lastlogintime >0 ? date('Y-m-d',$item.lastlogintime) : '未登录'}</td>
                <td align="center">{eq name="item.status" value="1"}是{else /}否{/eq}</td>
                <td align="center">
                    
                    {gt name="item.userid" value="1"}
                        <a class="btn btn-green" href="{:url('System/adminEdit?userid='.$item->userid)}" data-toggle="dialog" mask="true" data-width="550" data-height="200"><span>修改</span></a> | <a title="密码将重置为1q2w3e4" class="btn btn-green" href="{:url('System/adminResetPassword?userid='.$item->userid)}" data-toggle="doajax" data-confirm-msg="确定要重置密码吗？"><span>重置密码</span></a> | <a class="btn btn-green" href="{:url('System/adminChangeStatus?userid='.$item->userid)}" data-toggle="doajax"><span>{eq name="item.status" value="0"}启用{else /}禁用{/eq}</span></a> | <a class="btn btn-red" href="{:url('System/adminDelete?userid='.$item->userid)}" data-toggle="doajax" data-confirm-msg="确定要删除该管理员吗？"><span>删除</span></a>
                    {else /}
...
<div class="bjui-pageFooter">
    <div class="pages">
        ...
        <span>&nbsp;条，共 {$page.totalCount|default=""} 条</span>
    </div>
    <div class="pagination-box" data-toggle="pagination" data-total="{$page.totalCount|default=""}" data-page-size="{$page.pageSize|default=""}" data-page-current="{$page.pageCurrent|default=""}"></div>
</div>