        ...
        $box.find('[data-toggle="kindeditor"]').each(function() {
            ...
            KindEditor.create($editor, {
                ...
                cssPath                  : [
                                                ...
                                                BJUI.PLUGINPATH + 'kindeditor_4.1.10/img-width.css',
                                                BJUI.PLUGINPATH + 'kindeditor_4.1.10/plugins/code/prettify.css'
                                           ],
                afterBlur                : function() { this.sync() },
				afterCreate : function()		//html粘贴
				{
					var doc = this.edit.doc; 
					var cmd = this.edit.cmd; 
					if(!KindEditor.WEBKIT && !KindEditor.GECKO)
					{
						var pasted = false;
						...
						KindEditor(doc.body).bind('paste', function(ev)
						{
							setTimeout(function()
							{
								...
								if(html.search(/<img src="data:.+;base64,/) > -1)
								{
									$.post(createLink('file', 'ajaxPasteImage'), {editor: html}, function(data){
                                        if(data.flag){
                                            KindEditor(doc.body).html(data.data);
                                        }else{
                                            alert(data.msg);
                                        }
                                    });
